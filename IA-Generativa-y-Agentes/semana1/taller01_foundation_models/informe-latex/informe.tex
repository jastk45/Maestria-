...
También consideramos que el problema estaba en español y GPT-2 se entrenó
...
\input{tablas/p1}Elegiríamos gpt-5.6-luna: acertó los diez casos y costó aproximadamente 0,00064
dólares en total. Qwen acertó nueve sin cobro de API, pero tardó más y
necesitó recursos locales. Ampliaríamos las pruebas antes de usar cualquiera en
producción.

Confirmamos que los precios de la tabla se leyeron de la tabla semestral del
...
\input{tablas/p2bk}Encontramos doce configuraciones con 0 \% de exactitud y tres con 3,33 \%. Ninguna
fue suficiente para esta tarea. Usaríamos T=0 y top\_p=1 como punto de partida por
su estabilidad, pero tendríamos que mejorar el prompt o cambiar de modelo. Para
titulares probaríamos T=1 y top\_p=0.9, evaluándolos por separado. Al bajar la
...
secuenciales, no en los diez. Ejecutamos tres repeticiones por caso y
...
respuesta correcta; usamos el modo JSON del proveedor, sin esquema estricto.
Elegiríamos CoT para estos ejercicios, few-shot para mostrar criterios mediante
...
razonamiento; medium, 288; y high, 375. De medium a high pagamos 0,0001044
dólares adicionales sin mejorar la exactitud. Elegiríamos low para este conjunto.
La similitud entre low y medium no demuestra que el parámetro se ignore;
necesitaríamos más casos y repeticiones.
...
\input{tablas/p4b}No observamos empeoramiento: ambos niveles acertaron los tres casos. No reprodujimos
...
Entendemos la atención como una forma de combinar información relevante del
...
Una cada una de 43 sillas. ¿Cuántos tornillos le sobran?
\end{verbatim}En nuestra prueba, GPT-2 continuó el enunciado sin resolverlo. El ajuste con
instrucciones y preferencias busca mejorar la respuesta a peticiones. Para
estas aplicaciones necesitamos seguir instrucciones y respetar formatos, pero
...
Pagaríamos CoT en estos ejercicios porque pasó de uno a diez aciertos. Para
extraer un dato explícito, primero probaríamos una instrucción corta. Decidiríamos
...
Consideramos que retirar un parámetro puede simplificar el servicio y limitar
...
No asumiríamos que una explicación convincente refleja cómo se obtuvo la
respuesta. Lanham et al. encontraron que la influencia de la cadena cambia
según la tarea y el modelo. Para auditar, combinaríamos las trazas con casos
