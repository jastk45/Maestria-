...
=> $item, '
\end{verbatim}
Con \texttt{do\_sample=True} y \texttt{top\_k=1}, las 25 corridas registradas en el CSV (5 ejecuciones del notebook con 5 semillas cada una) produjeron 1 salida distinta: idéntica a la de greedy (Tabla~\ref{tab:p0k1}).\input{tablas/p0k1}
...
\input{tablas/p1}Elegiríamos gpt-5.6-luna: acertó los diez casos, fue el más rápido (1,13 s
frente a 1,91 s de gpt-4o-mini y 10,7 s de qwen3 en local) y costó 0,00064 USD
por los diez. Qwen3 acertó nueve a costo cero, pero requiere recursos locales y tarda porque
...
\input{tablas/p2a}Mensajes literales completos de las celdas rechazadas:
...
\input{tablas/p2bk}La temperatura divide los logits antes del softmax: con T<1 amplía la
...
top\_p=1, por estabilidad, ya que ninguna celda mejora la exactitud: la única palanca que la mueve es el prompt o el modelo.
Para tres titulares alternativos elegiríamos T=1 con top\_p=0,9: hay
dispersión y el núcleo descarta tokens de menor probabilidad. En local, top\_k=1 dio la misma salida en
ocho de diez casos en cinco corridas secuenciales, no en los diez, y la
dispersión creció con k (estabilidad 0,96 → 0,86 de top\_k=1 a 40) y la exactitud
observada pasó de 92 \% a 98 \%.
...
{"respuesta": 71}
\end{verbatim}
Zero-shot acertó un caso y few-shot ninguno. En estos casos, pedir directamente un número produjo menor exactitud que
solicitar pasos intermedios; los ejemplos duplican los tokens de entrada (1 842 frente a 793 en los diez casos)
...
razonamiento medidos fueron 287, 288 y 375 en los diez casos. Low y medium tuvieron consumos casi iguales en esta ejecución. No
encontramos una mejora de exactitud al aumentar el esfuerzo, pero
necesitaríamos más repeticiones para evaluar su efecto. High sí
...
podemos calcular un costo por punto. Low fue suficiente entre los niveles probados; no evaluamos niveles
inferiores. Las cifras también corrigen el
supuesto del presupuesto: 38,8 tokens de salida totales por llamada en low
frente a los 1 500 que supone el enunciado, 38,7 veces menos; el costo
estimado con ese supuesto era 0,020 USD y el real fue 0,00062 USD, 32,1
veces menos. La lección no es que el dial no sirva, sino que el nivel se
...
intermedios que una instrucción directa no resolvió, cuando el costo de un
